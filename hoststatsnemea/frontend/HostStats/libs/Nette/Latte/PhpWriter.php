...
class PhpWriter extends Object
{
	...
	public function write($mask)
	{
		...
		$me = $this;
		$mask = Strings::replace($mask, '#%escape(\(([^()]*+|(?1))+\))#', new Callback(create_function('$m', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('me'=>$me)).'-1], EXTR_REFS);
			return $me->escape(substr($m[1], 1, -1));
		')));
		$mask = Strings::replace($mask, '#%modify(\(([^()]*+|(?1))+\))#', new Callback(create_function('$m', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('me'=>$me)).'-1], EXTR_REFS);
			return $me->formatModifiers(substr($m[1], 1, -1));
		')));

		return Strings::replace($mask, '#([,+]\s*)?%(node\.word|node\.array|node\.args|var|raw)(\?)?(\s*\+\s*)?()#',
			new Callback(create_function('$m', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('me'=>$me,'word'=> $word, 'args'=>& $args)).'-1], EXTR_REFS);
