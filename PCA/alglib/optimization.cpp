...
static void minqp_minqpgrad(minqpstate* state,
     /* Real    */ ae_vector* xc,
     /* Real    */ ae_vector* gc,
     ae_state *_state);
static void minqp_minqpscalegrad(minqpstate* state,
     /* Real    */ ae_vector* g,
     ae_state *_state);
static double minqp_minqpxtax2(minqpstate* state,
     /* Real    */ ae_vector* x,
     ae_state *_state);
static void minqp_minqptargetfunction(minqpstate* state,
     /* Real    */ ae_vector* x,
     double* f,
     double* noise,
     ae_state *_state);
static ae_bool minqp_minqppreparenewtonstep(minqpstate* state,
     /* Real    */ ae_vector* xc,
     ...
     /* Real    */ ae_vector* bufc,
     ae_state *_state);
static void minqp_minqpstepto(minqpstate* state,
     /* Real    */ ae_vector* xc,
     /* Real    */ ae_vector* xn,
     /* Real    */ ae_vector* bndl,
     ...
     /* Real    */ ae_vector* tmp0,
     /* Real    */ ae_vector* tmp1,
     ae_bool* constraintsactivated,
     ae_bool* constraintsdeactivated,
     ae_bool* noisystep,
     ae_state *_state);
     ...
     ae_state *_state)
{
    ...
    for(i=0; i<=nmain-1; i++)
    {
        if( ae_fp_neq(x->ptr.p_double[i],xprev->ptr.p_double[i])&&((havebndl->ptr.p_bool[i]&&ae_fp_eq(x->ptr.p_double[i],bndl->ptr.p_double[i]))||(havebndu->ptr.p_bool[i]&&ae_fp_eq(x->ptr.p_double[i],bndu->ptr.p_double[i]))) )
        {
            result = result+1;
        }
    }
    for(i=0; i<=nslack-1; i++)
    {
        if( ae_fp_neq(x->ptr.p_double[nmain+i],xprev->ptr.p_double[nmain+i])&&ae_fp_eq(x->ptr.p_double[nmain+i],0.0) )
        {
            result = result+1;
        }
    }
...
void minqpcreate(ae_int_t n, minqpstate* state, ae_state *_state)
{
    ...
    ae_vector_set_length(&state->xcand0, n, _state);
    ae_vector_set_length(&state->xcand1, n, _state);
    ae_vector_set_length(&state->xc, n, _state);
    ae_vector_set_length(&state->xn, n, _state);
    ae_vector_set_length(&state->gc, n, _state);
    ae_vector_set_length(&state->pg, n, _state);
    ae_vector_set_length(&state->dc, n, _state);
    ae_vector_set_length(&state->activeset, n, _state);
    for(i=0; i<=n-1; i++)
    {
    ...
    if( state->algokind==1&&state->akind==0 )
    {
        ...
        if( nbc==0&&nlc==0 )
        {
            ...
            for(i=0; i<=n-1; i++)
            {
                for(j=i; j<=n-1; j++)
                {
                    state->bufa.ptr.pp_double[i][j] = state->densea.ptr.pp_double[i][j];
                }
            }
            ae_v_move(&state->bufb.ptr.p_double[0], 1, &state->b.ptr.p_double[0], 1, ae_v_len(0,n-1));
            state->repncholesky = 1;
            if( !spdmatrixcholeskyrec(&state->bufa, 0, n, ae_true, &state->tmp0, _state) )
            {
                state->repterminationtype = -5;
                return;
            }
            fblscholeskysolve(&state->bufa, 1.0, n, ae_true, &state->bufb, &state->tmp0, _state);
        ...
        ae_assert(nlc==0, "MinQPOptimize: internal error", _state);
        ...
        for(;;)
        {
             ...
             * Phase 1.
             * 
             * Determine active set using only current point location
             * (we ignore antigradient direction, constraint is activated
             * when current point is located at the bound of the area).
             *
            ...
            for(i=0; i<=n-1; i++)
            {
                state->activeset.ptr.p_bool[i] = (state->havebndl.ptr.p_bool[i]&&ae_fp_eq(state->xc.ptr.p_double[i],state->workbndl.ptr.p_double[i]))||(state->havebndu.ptr.p_bool[i]&&ae_fp_eq(state->xc.ptr.p_double[i],state->workbndu.ptr.p_double[i]));
            }
            state->repncholesky = state->repncholesky+1;
            if( !minqp_minqppreparenewtonstep(state, &state->xc, &state->activeset, &state->xn, n, &state->bufa, &state->tmp0, &state->tmp1, _state) )
            {
                state->repterminationtype = -5;
                return;
            }
            minqp_minqpstepto(state, &state->xc, &state->xn, &state->workbndl, &state->havebndl, &state->workbndu, &state->havebndu, n, &state->tmp0, &state->tmp1, &constraintsactivated, &constraintsdeactivated, &noisystep, _state);
            ae_v_move(&state->xc.ptr.p_double[0], 1, &state->xn.ptr.p_double[0], 1, ae_v_len(0,n-1));
            
            /*
             * Phase 2.
             * 
             * Determine active set using current point location and
             * antigradient direction. Then perform steepest descent step.
             *
             * 1. we calculate projected gradient PG and scale using
             ...
             */
            minqp_minqpgrad(state, &state->xc, &state->gc, _state);
            ae_v_move(&state->pg.ptr.p_double[0], 1, &state->gc.ptr.p_double[0], 1, ae_v_len(0,n-1));
            minqp_minqpscalegrad(state, &state->pg, _state);
            projectgradientintobc(&state->xc, &state->pg, &state->workbndl, &state->havebndl, &state->workbndu, &state->havebndu, n, 0, _state);
            minqp_minqptargetfunction(state, &state->xc, &v, &vn0, _state);
            v0 = minqp_minqpxtax2(state, &state->pg, _state);
            if( ae_fp_greater(v0,0) )
            {
                ...
                ae_v_subd(&state->xn.ptr.p_double[0], 1, &state->pg.ptr.p_double[0], 1, ae_v_len(0,n-1), v);
                minqp_minqpstepto(state, &state->xc, &state->xn, &state->workbndl, &state->havebndl, &state->workbndu, &state->havebndu, n, &state->tmp0, &state->tmp1, &constraintsactivated, &constraintsdeactivated, &noisystep, _state);
                ae_v_move(&state->xc.ptr.p_double[0], 1, &state->xn.ptr.p_double[0], 1, ae_v_len(0,n-1));
            }
            else
             ...
             * 
             * Determine active set using current point location and
             * antigradient direction. Then perform Newton step.
             *
             * This phase allows us to leave current face.
             */
            minqp_minqpgrad(state, &state->xc, &state->gc, _state);
            for(i=0; i<=n-1; i++)
            {
                state->activeset.ptr.p_bool[i] = ae_false;
                state->activeset.ptr.p_bool[i] = state->activeset.ptr.p_bool[i]||((state->havebndl.ptr.p_bool[i]&&ae_fp_eq(state->xc.ptr.p_double[i],state->workbndl.ptr.p_double[i]))&&ae_fp_greater(state->gc.ptr.p_double[i],0));
                state->activeset.ptr.p_bool[i] = state->activeset.ptr.p_bool[i]||((state->havebndu.ptr.p_bool[i]&&ae_fp_eq(state->xc.ptr.p_double[i],state->workbndu.ptr.p_double[i]))&&ae_fp_less(state->gc.ptr.p_double[i],0));
            }
            if( !minqp_minqppreparenewtonstep(state, &state->xc, &state->activeset, &state->xn, n, &state->bufa, &state->tmp0, &state->tmp1, _state) )
            {
                state->repterminationtype = -5;
     ...
     ae_state *_state)
{
    ...
    if( isupper )
    {
        for(k=0; k<=n-2; k++)
        {
            state->densea.ptr.pp_double[k][k] = a->ptr.pp_double[k][k]+s;
            ae_v_move(&state->densea.ptr.pp_double[k+1][k], state->densea.stride, &a->ptr.pp_double[k][k+1], 1, ae_v_len(k+1,n-1));
            ae_v_move(&state->densea.ptr.pp_double[k][k+1], 1, &a->ptr.pp_double[k][k+1], 1, ae_v_len(k+1,n-1));
        }
        state->densea.ptr.pp_double[n-1][n-1] = a->ptr.pp_double[n-1][n-1]+s;
    }
    else
    {
        state->densea.ptr.pp_double[0][0] = a->ptr.pp_double[0][0]+s;
        for(k=1; k<=n-1; k++)
        {
            ae_v_move(&state->densea.ptr.pp_double[k][0], 1, &a->ptr.pp_double[k][0], 1, ae_v_len(0,k-1));
            ae_v_move(&state->densea.ptr.pp_double[0][k], state->densea.stride, &a->ptr.pp_double[k][0], 1, ae_v_len(0,k-1));
            state->densea.ptr.pp_double[k][k] = a->ptr.pp_double[k][k]+s;
        }
    }
}
     ...
     ae_state *_state)
{
    ...
    for(k=0; k<=n-1; k++)
    {
        state->densea.ptr.pp_double[k][k] = s->ptr.p_double[k];
    }
}
     ...
     ae_state *_state)
{
    ...
    if( state->akind==-1 )
    {
        ae_v_move(&gc->ptr.p_double[0], 1, &state->b.ptr.p_double[0], 1, ae_v_len(0,n-1));
        return;
    }
    ...
    {
        ae_v_move(&gc->ptr.p_double[0], 1, &state->b.ptr.p_double[0], 1, ae_v_len(0,n-1));
        gc->ptr.p_double[0] = gc->ptr.p_double[0]+state->densea.ptr.pp_double[0][0]*xc->ptr.p_double[0];
        for(i=1; i<=n-1; i++)
        {
            v = ae_v_dotproduct(&state->densea.ptr.pp_double[i][0], 1, &xc->ptr.p_double[0], 1, ae_v_len(0,i-1));
            gc->ptr.p_double[i] = gc->ptr.p_double[i]+v+state->densea.ptr.pp_double[i][i]*xc->ptr.p_double[i];
            v = xc->ptr.p_double[i];
            ae_v_addd(&gc->ptr.p_double[0], 1, &state->densea.ptr.pp_double[i][0], 1, ae_v_len(0,i-1), v);
        }
        return;
    }
}
...
/*************************************************************************
This function scales gradient by multiplying it by inverse of the diagonal
...
It MUST be called only for problems with SPD matrices.
     ...
     ae_state *_state)
{
    ...
    n = state->n;
    ae_assert(state->akind==0, "MinQPGrad: internal error", _state);
    ...
    if( state->akind==0 )
    {
        for(i=0; i<=n-1; i++)
        {
            g->ptr.p_double[i] = g->ptr.p_double[i]/state->densea.ptr.pp_double[i][i];
        }
        return;
    }
}
...
/*************************************************************************
This  function  calculates 0.5*x'*A*x for given X.
     ...
     ae_state *_state)
{
    ...
    if( state->akind==0 )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            ...
            result = result+0.5*state->densea.ptr.pp_double[i][i]*ae_sqr(x->ptr.p_double[i], _state);
        }
        return result;
    }
    return result;
...
/*************************************************************************
This function calculates 0.5*x'*A*x+b*x for given X.

INPUT PARAMETERS:
    State   -   quadratic model
    X       -   point of calculation
    
OUTPUT PARAMETERS:
    F       -   function value
    Noise   -   estimate of the numerical noise level
    ...
    *noise = 0;
    ...
    if( state->akind==-1 )
    {
        ...
        *noise = 0.0;
        ...
        for(i=0; i<=n-1; i++)
        {
            v = state->b.ptr.p_double[i]*x->ptr.p_double[i];
            *f = *f+v;
            *noise = ae_maxreal(*noise, ae_fabs(v, _state), _state);
        }
        
        /*
         * End
         */
        *noise = ae_machineepsilon*n*(*noise);
        return;
    }
    ...
    if( state->akind==0 )
    {
        ...
        *noise = 0;
        ...
        for(i=0; i<=n-1; i++)
        {
            for(j=0; j<=i-1; j++)
            {
                v = state->densea.ptr.pp_double[i][j]*x->ptr.p_double[i]*x->ptr.p_double[j];
                *f = *f+v;
                *noise = ae_maxreal(*noise, ae_fabs(v, _state), _state);
            }
            v = 0.5*state->densea.ptr.pp_double[i][i]*ae_sqr(x->ptr.p_double[i], _state);
            *f = *f+v;
            *noise = ae_maxreal(*noise, ae_fabs(v, _state), _state);
        }
...
/*************************************************************************
This function calculates Newton step subject to the set of the active
equality constraints.

INPUT PARAMETERS:
    State   -   MinQP state. This structure is used ONLY to get quadratic
                model. This function does NOT use  other  fields  of  the
                State structure (like temporary buffers or bounds).
    XC      -   current point
    ActiveSet-  set of active equality constraints
    XN      -   preallocated buffer
    N       -   problem size
    BufA    -   temporary buffer (automatically reallocated, if needed)
    BufB    -   temporary buffer (automatically reallocated, if needed)

OUTPUT PARAMETERS:
    XN      -   minimizer  of  the  quadratic  model  subject to equality
                constraints. This point can be infeasible because full
                Newton step may lead us to infeasible area.

RESULT:
    True, when Newton step was correctly calculated
    False, when this function was called for non-convex problem (matrix
    is not positive definite, Cholesky factorization failed)
                
NOTE: this function correctly works when all constraints are active.
     ...
     ae_state *_state)
{
    ...
    ae_assert(state->akind==0, "MinQPNewtonStep: unexpected model type", _state);
    
    /*
     * Determine XN - minimizer of the quadratic function subject to active constraints.
     * It is done in two substeps:
     * * first, we determine values of the constrained components
     * * then we determine unconstrained components by splitting x'*A*x+b'*x into
     *               ( A0  A1 ) ( x0 )              ( x0 )
     *   ( x0' x1' )*(        )*(    ) + (b0' b1')* (    ), with fixed x1 and "free" x0,
     *               ( A1' A2 ) ( x1 )              ( x1 )
     *   and seeking for minimizer x0_min = inv(A0)*(-A1*x1-b0). We store A0 into BufA,
     *   and -A1*x1-b0 is stored into BufB. We make implicit permutation of variables
     *   when storing these matrices/vectors and retreiving result into XN.
     */
    rmatrixsetlengthatleast(bufa, n, n, _state);
    rvectorsetlengthatleast(bufb, n, _state);
    for(i=0; i<=n-1; i++)
    {
        if( activeset->ptr.p_bool[i] )
        {
            xn->ptr.p_double[i] = xc->ptr.p_double[i];
        }
    }
    nfree = 0;
    for(i=0; i<=n-1; i++)
    {
        if( !activeset->ptr.p_bool[i] )
        {
            bufb->ptr.p_double[nfree] = -state->b.ptr.p_double[i];
            k = 0;
            for(j=0; j<=n-1; j++)
            {
                if( !activeset->ptr.p_bool[j] )
                {
                    bufa->ptr.pp_double[nfree][k] = state->densea.ptr.pp_double[i][j];
                    k = k+1;
                }
                else
                {
                    bufb->ptr.p_double[nfree] = bufb->ptr.p_double[nfree]-state->densea.ptr.pp_double[i][j]*xn->ptr.p_double[j];
                }
            }
            nfree = nfree+1;
        }
    ...
    if( nfree>0 )
    {
        if( !spdmatrixcholeskyrec(bufa, 0, nfree, ae_true, bufc, _state) )
        {
            result = ae_false;
            return result;
        }
        fblscholeskysolve(bufa, 1.0, nfree, ae_true, bufb, bufc, _state);
        k = 0;
        for(i=0; i<=n-1; i++)
        {
            if( !activeset->ptr.p_bool[i] )
            {
                xn->ptr.p_double[i] = bufb->ptr.p_double[k];
                k = k+1;
            }
        }
    }
...
/*************************************************************************
This function performs step from XC to XN subject to bound constraints and
on  condition  that  function  value  decreases. User prepares two points:
* XC, current point, feasible w.r.t. bound constraints
* XN, candidate point, which minimizes function value in some sense (either
  it is Newton  minimizer, or minimizer along steepest descent  direction),
  subject to equality constraints active at XC. It can be infeasible w.r.t.
  other  constraints  -  ones  which were not active at XC.

It is important that function MUST monotonically decrease all the way from
XC to XN.

This function:
1. compares quadratic model values at XN and XC; if f(XN)>=f(XC), it sets
   NoisyStep, sets XN to XC and returns
2. performs longest step from XC to XN until we activate constraints
3. handles the rest of the step  by  performing  Armijo  line  search  for
   f()=PROJ(XC_new+alpha*(XN-XC_new)) where  PROJ()  is  a  projection  on
   bound constrained area, alpha is a  search  parameter  (we  start  from
   alpha=1 and decrease it), and XC_new is a new point obtained at step 2.
4. New location is saved into XN. In case some constraints were  activated
   or deactivated, we set corresponding flag variable.  In  case  function
   value  decrease  is  dominated by the numerical noise, we set NoisyStep
   flag.
...
INPUT PARAMETERS:
    State   -   MinQP state. This structure is used ONLY to get quadratic
                model. This function does NOT use  other  fields  of  the
                State structure (like temporary buffers or bounds).
    XC      -   current point, must be feasible w.r.t. constraints
    XN      -   candidate point, can be infeasible
    BndL    -   lower bounds, array[N]
                (may contain -INF, when bound is not present)
    HaveBndL-   array[N], if HaveBndL[i] is False,
                then i-th bound is not present
    BndU    -   array[NMain], upper bounds
                (may contain +INF, when bound is not present)
    HaveBndU-   array[NMain], if HaveBndU[i] is False,
                then i-th bound is not present
    N       -   problem size
    Tmp0    -   temporary buffer (automatically reallocated if needed)

OUTPUT PARAMETERS:
    XN      -   new point, feasible w.r.t. bound constraints
  
NOTE 1: it is important that XN is constrained only by constraints,  which
        are active at XC. Any attempt to introduce additional  constraints
        may lead to algorithm stagnation
     ...
     ae_state *_state)
{
    ...
    *constraintsactivated = ae_false;
    *constraintsdeactivated = ae_false;
    *noisystep = ae_false;
    ...
    /*
     * Compare function values at XC and XN
     */
    minqp_minqptargetfunction(state, xc, &v0, &v, _state);
    minqp_minqptargetfunction(state, xn, &v1, &v, _state);
    if( ae_fp_greater_eq(v1,v0) )
    {
        *constraintsactivated = ae_false;
        *constraintsdeactivated = ae_false;
        *noisystep = ae_true;
        ae_v_move(&xn->ptr.p_double[0], 1, &xc->ptr.p_double[0], 1, ae_v_len(0,n-1));
        return;
    }
    
    /*
     * Perform bounded step (we activate at most one constraint during this phase)
     */
    ae_v_move(&tmp0->ptr.p_double[0], 1, &xn->ptr.p_double[0], 1, ae_v_len(0,n-1));
    ae_v_sub(&tmp0->ptr.p_double[0], 1, &xc->ptr.p_double[0], 1, ae_v_len(0,n-1));
    calculatestepbound(xc, tmp0, 1.0, bndl, havebndl, bndu, havebndu, n, 0, &variabletofreeze, &valuetofreeze, &maxsteplen, _state);
    if( variabletofreeze>=0&&ae_fp_less_eq(maxsteplen,1) )
    {
        ...
        ae_v_move(&tmp0->ptr.p_double[0], 1, &xn->ptr.p_double[0], 1, ae_v_len(0,n-1));
        ae_v_sub(&tmp0->ptr.p_double[0], 1, &xc->ptr.p_double[0], 1, ae_v_len(0,n-1));
        ae_v_muld(&tmp0->ptr.p_double[0], 1, ae_v_len(0,n-1), maxsteplen);
        ae_v_add(&tmp0->ptr.p_double[0], 1, &xc->ptr.p_double[0], 1, ae_v_len(0,n-1));
        postprocessboundedstep(tmp0, xc, bndl, havebndl, bndu, havebndu, n, 0, variabletofreeze, valuetofreeze, maxsteplen, maxsteplen, _state);
    }
    else
    {
    ...
    /*
     * Tmp0 contains either XN - or intermediate point halfway between XC and XN.
     * In case it contains intermediate point, handle the rest using Armijo search and projections.
     */
    if( variabletofreeze>=0&&ae_fp_less_eq(maxsteplen,1) )
    {
        ...
        /*
         * Armijo search.
         * After this block Tmp1 contains best point found so far (can be equal to Tmp0).
         */
        minqp_minqptargetfunction(state, tmp0, &v0, &v, _state);
        armijostep = 1.0;
        armijocnt = 5;
        ae_v_move(&tmp1->ptr.p_double[0], 1, &tmp0->ptr.p_double[0], 1, ae_v_len(0,n-1));
        while(armijocnt>0)
        {
            
            /*
             * Store candidate point in Tmp1
             */
            ae_v_move(&tmp1->ptr.p_double[0], 1, &xn->ptr.p_double[0], 1, ae_v_len(0,n-1));
            ae_v_sub(&tmp1->ptr.p_double[0], 1, &tmp0->ptr.p_double[0], 1, ae_v_len(0,n-1));
            ae_v_muld(&tmp1->ptr.p_double[0], 1, ae_v_len(0,n-1), armijostep);
            ae_v_add(&tmp1->ptr.p_double[0], 1, &tmp0->ptr.p_double[0], 1, ae_v_len(0,n-1));
            for(i=0; i<=n-1; i++)
            {
                if( havebndl->ptr.p_bool[i]&&ae_fp_less(tmp1->ptr.p_double[i],bndl->ptr.p_double[i]) )
                {
                    tmp1->ptr.p_double[i] = bndl->ptr.p_double[i];
                }
                if( havebndu->ptr.p_bool[i]&&ae_fp_greater(tmp1->ptr.p_double[i],bndu->ptr.p_double[i]) )
                {
                    tmp1->ptr.p_double[i] = bndu->ptr.p_double[i];
                }
            }
             ...
             * Evaluate function
             */
            minqp_minqptargetfunction(state, tmp1, &v1, &v, _state);
            if( ae_fp_greater_eq(v1,v0) )
            {
                ae_v_move(&tmp1->ptr.p_double[0], 1, &tmp0->ptr.p_double[0], 1, ae_v_len(0,n-1));
            }
             ...
             * Next iteration
             */
            armijocnt = armijocnt-1;
            armijostep = 0.5*armijostep;
        }
        ae_v_move(&xn->ptr.p_double[0], 1, &tmp1->ptr.p_double[0], 1, ae_v_len(0,n-1));
    }
    else
    {
         ...
         * No Armijo search
         */
        ae_v_move(&xn->ptr.p_double[0], 1, &tmp0->ptr.p_double[0], 1, ae_v_len(0,n-1));
    }
    ...
    *constraintsdeactivated = ae_false;
    for(i=0; i<=n-1; i++)
    {
        b0 = (havebndl->ptr.p_bool[i]&&ae_fp_eq(bndl->ptr.p_double[i],xc->ptr.p_double[i]))||(havebndu->ptr.p_bool[i]&&ae_fp_eq(bndu->ptr.p_double[i],xc->ptr.p_double[i]));
        b1 = (havebndl->ptr.p_bool[i]&&ae_fp_eq(bndl->ptr.p_double[i],xn->ptr.p_double[i]))||(havebndu->ptr.p_bool[i]&&ae_fp_eq(bndu->ptr.p_double[i],xn->ptr.p_double[i]));
        *constraintsactivated = *constraintsactivated||(b1&&!b0);
        *constraintsdeactivated = *constraintsdeactivated||(b0&&!b1);
    }
    minqp_minqptargetfunction(state, xc, &v0, &vn0, _state);
    minqp_minqptargetfunction(state, xn, &v1, &vn1, _state);
    *noisystep = ae_fp_less(v0-v1,ae_maxreal(vn0, vn1, _state));
}
...
ae_bool _minqpstate_init(minqpstate* p, ae_state *_state, ae_bool make_automatic)
{
    ...
    if( !ae_vector_init(&p->gc, 0, DT_REAL, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init(&p->dc, 0, DT_REAL, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init(&p->xcand0, 0, DT_REAL, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init(&p->xcand1, 0, DT_REAL, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init(&p->xn, 0, DT_REAL, _state, make_automatic) )
        return ae_false;
...
ae_bool _minqpstate_init_copy(minqpstate* dst, minqpstate* src, ae_state *_state, ae_bool make_automatic)
{
    ...
    if( !ae_vector_init_copy(&dst->gc, &src->gc, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init_copy(&dst->dc, &src->dc, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init_copy(&dst->xcand0, &src->xcand0, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init_copy(&dst->xcand1, &src->xcand1, _state, make_automatic) )
        return ae_false;
    if( !ae_vector_init_copy(&dst->xn, &src->xn, _state, make_automatic) )
        return ae_false;
...
void _minqpstate_clear(minqpstate* p)
{
    ...
    ae_vector_clear(&p->xcand0);
    ae_vector_clear(&p->xcand1);
    ae_vector_clear(&p->xn);
