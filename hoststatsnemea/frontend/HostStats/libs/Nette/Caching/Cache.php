...
class Cache extends Object implements ArrayAccess
{
	...
	public function call($function)
	{
		...
		return $this->load($key, create_function('', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('function'=>$function,'key'=> $key)).'-1], EXTR_REFS);
			array_shift($key);
			return Callback::create($function)->invokeArgs($key);
		'));
	}
	...
	{
		$cache = $this;
		return create_function('', 'extract($GLOBALS[0]['.array_push($GLOBALS[0], array('cache'=>$cache,'function'=> $function,'dp'=> $dp)).'-1], EXTR_REFS);
